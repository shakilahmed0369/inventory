<x-admin-layout>
    ...
    <x-admin.card title="Product details" description="Update the information below for {{ $product->name }}.">
        <form method="POST" action="{{ route('products.update', $product) }}" enctype="multipart/form-data" class="space-y-5">
            ...
            <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                <div>
                    ...
                    <x-admin.input id="name" name="name" type="text" :value="old('name', $product->name)"
                                   placeholder="e.g. Widget Pro" autofocus />
                    <x-admin.input-error :messages="$errors->get('name')" />
                </div>
                <div>
                    <x-admin.label for="sku" value="SKU" />
                    <x-admin.input id="sku" name="sku" type="text" :value="old('sku', $product->sku)"
            ...
            <div>
                ...
                @if ($product->image)
                    <div class="mb-2 flex items-center gap-3">
                        <img src="{{ Storage::url($product->image) }}"
                             class="size-16 rounded-md object-cover border border-zinc-200" alt="">
                        <p class="text-xs text-zinc-500">Current image. Upload a new one to replace it.</p>
                    </div>
                @endif
                ...
                <p class="mt-1 text-xs text-zinc-400">PNG, JPG, GIF up to 2 MB. Leave empty to keep existing image.</p>
            ...
            <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                <div>
                    ...
                    <x-admin.input id="purchase_price" name="purchase_price" type="number" step="0.01" min="0"
                                   :value="old('purchase_price', $product->purchase_price)" placeholder="0.00" />
                ...
                <div>
                    ...
                    <x-admin.input id="sell_price" name="sell_price" type="number" step="0.01" min="0"
                                   :value="old('sell_price', $product->sell_price)" placeholder="0.00" />
                    <x-admin.input-error :messages="$errors->get('sell_price')" />
                </div>
            </div>

            {{-- Current stock (read-only — managed by sales) --}}
            <div class="sm:w-1/2">
                <x-admin.label for="current_stock" value="Current Stock (units)" />
                <x-admin.input id="current_stock" type="number" :value="$product->current_stock"
                               class="bg-zinc-50" disabled />
                <p class="mt-1 text-xs text-zinc-400">Stock is adjusted automatically when sales are recorded.</p>
            </div>
